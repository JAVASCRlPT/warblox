            ...
            <div class="card-body p-4">
                <form method="POST" action="{{ route('admin.books.update', $book) }}" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control @error('year') is-invalid @enderror" 
                                   id="year" name="year" value="{{ old('year', $book->year) }}" min="1900" max="{{ date('Y') }}" required>
                            @error('year')
                            <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control @error('price') is-invalid @enderror" 
                                   id="price" name="price" value="{{ old('price', $book->price) }}" min="0">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('loan_duration') is-invalid @enderror" 
                                   id="loan_duration" name="loan_duration" value="{{ old('loan_duration', $book->loan_duration) }}" min="1" max="365" required>
                    ...
                    <div class="mb-3">
                        ...
                        @if($book->file_ebook)
                        <div class="mb-2">
                            <small class="text-muted">File saat ini: {{ pathinfo($book->file_ebook, PATHINFO_BASENAME) }}</small>
                        </div>
                        @endif
                        <input type="file" class="form-control @error('file_ebook') is-invalid @enderror" 
                               id="file_ebook" name="file_ebook" accept=".pdf,.epub">
                        <small class="text-muted">Unggah file eBook jika ingin mengganti.</small>
                        @error('file_ebook')
                    ...
                    <div class="mb-3">
                        ...
                        @if($book->cover)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $book->cover) }}" alt="Cover" class="img-fluid rounded" style="max-height: 150px;">
                            <br>
                        ...
                        <small class="text-muted">Format: JPEG, PNG, GIF. Max 2MB. Biarkan kosong jika tidak ingin mengubah.</small>
                        @error('cover')
                        <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
